...
<?php
$sql = "SELECT * FROM mylokalfood.order_invoice WHERE cust_id = '$cust_id' ORDER BY order_id DESC";
...
<?php
	$sql2 = "SELECT food.food_id, food.food_name, order_line.food_qty
	FROM lokalfood.order_line, lokalfood.food WHERE
...
?>		
	...
	<tr>
		...
		if ($reviewFlag == "Y"){
			...
			echo '<a href="review.php?food-id=' . $row2["food_id"] . '&order-id='. $row1["order_id"]. '&food-name='. $row2["food_name"].'">' . '<i class="fa fa-comment"></i> Review</a>';
		}
		?>
